...
export default function CreateSetClient({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post(`/api/admin/tests/${testId}/sets`, {
        ...form,
      ...
      if (err instanceof AxiosError) {
        if (err.response?.status === 400) {
          ...
          setApiError(err.response?.data?.error || "Failed to create set. Try again.");
        }
      } else {
    ...
    <Container className="py-8 md:py-10 max-w-3xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-6 md:p-8 space-y-6 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Field label="Total Questions *" error={errors.totalQuestions}>
              <Input
                ...
                onChange={(e) => set("totalQuestions", e.target.value === "" ? "" : Number(e.target.value))}
          ...
          <Field label="Display Order *" error={errors.order} hint="Sets are shown to students in this order">
            <Input
              type="number"
              min={1}
              value={form.order}
          ...
          {form.negativeMarking && (
            ...
            >
              <Input
                ...
                onChange={(e) => set("negativeMarkFraction", Number(e.target.value))}
              />
            </Field>
          )}
        </div>
